...
@extends('layouts.app', ['title' => $order->name.'｜メッセージ｜マイメニュー','css'=>'mymenu'])
				...
				<div class="progress_box mid_sec">
					...
					<dl class="conditions">
						...
						<dd>
							...
							<div class="btn_box">
									@if($message->u_doui!=1)<a href="/mymenu/message/acept/{{$message->id}}" class="btn01">同意して契約</a> @endif
								@if($message->u_doui==1) <p>同意しました相手の同意をお待ちください。</p> @endif
								@if($message->u_doui!=1)
								<dl class="btn_mt tac condition_box">
									<dt><span class="more">新しい条件を提示する</span></dt>
									<dd class="hide_content">
										<form action="/mymenu/message/proposal/{{$message->id}}" method="POST">@csrf
											<table class="input_form">
												...
												<tr>
													...
													<td>
														<input type="date" name="date" value="{{old('date')}}">s
													</td>
												</tr>
											</table>
					...
					@elseif(($message->j_at && $message->j_at>$message->u_at) )
					<dl class="conditions">
						<dt>
							新しい条件が提示されています。
						</dt>
						<dd>
							...
							<div class="after_condition mode_before">
								...
								<p class="price"><span class="pay">支払総額：{{number_format($message->u_price*0.8)}}円</span>（システム利用料控除後のジュエラー受領報酬：{{number_format($message->u_price*0.8)}}円）</p>
					...
					<dl class="conditions">
						...
						<dd>
							<div class="btn_box">
							@if($message->j_doui==1)相手が同意しました<br><a href="/mymenu/message/acept/{{$message->id}}" class="btn01">同意して契約</a> @endif
						</div>
							<div class="after_condition">
					...
					<dl class="conditions">
						...
						<dd>
							...
							<div class="btn_box">
								...
								<a href="/mymenu/pay/{{$message->id}}" class="btn01">仮払いを行う</a>
							</div>
							<div class="txt_box">
					...
					<dl class="conditions">
						...
						<dd>
							<div class="after_condition">
								<p class="from"><a href="/user/detail/{{$message->user_id}}">{{$message->jeweller->name}}</a>さんより</p>
							...
							<div class="btn_box">
								...
								<a href="/mymenu/receive/{{$message->id}}" class="btn01">受け取り・検品しました</a>
							</div>
							<div class="txt_box">
								<ul>
									<li class="read">今後の流れ</li>
					...
					<dl class="conditions">
						...
						<dd>
							...
							<div class="btn_box">
								...
								<a href="/mymenu/assessment/{{$message->id}}" class="btn01">評価する</a>
							</div>
							<div class="txt_box">
								<ul>
									<li class="read">今後の流れ</li>
					...
					<ul class="message_li">
						...
						<li>
							...
							<div class="data">
								...
								@if($messageList->file)
								<div class="file">
									DL：<a href="/mymenu/message/dl/{{$messageList->id}}" download="{{$messageList->org_file_name}}">{{$messageList->org_file_name}}</a>
								</div>
								@endif
							</div>
						</li>
